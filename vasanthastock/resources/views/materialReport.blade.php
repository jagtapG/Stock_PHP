        ...
        <div class="col-lg-12">
            <section class="box ">
                ...
                <div class="content-body"> 
                    ...
                    <form method="post" action="{{url('datesearch')}}">
                        {{csrf_field()}}
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <table id="example" class="table table-striped dt-responsive display" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td><a href='{{url("centralStoreEdit/{$result->id}")}}' class="btn btn-primary">Edit</a></td>
                                        <td>{{$result->material_date}}</td>
                                        <td>{{$result->material_id}}</td>
                                        <td>{{$result->material_name}}</td>
                                        <td>{{$result->uom}}</td>
                                        <td>{{$result->material}}</td>
                                        <td>{{$result->material_description}}</td>
                                        <td>{{$result->material_group}}</td>
                                        <td>{{$result->material_location}}</td>
                                        <td>{{$result->add_quantity}}</td>
                                        <td>{{$result->issueqty}}</td>
                                        <td>{{$result->stock}}</td>
                                        <td>{{$result->alert_Quantity}}</td>
                                        <td>{{$result->vehicle_no}}</td>
                                        <td>{{$result->issue_place}}</td>
                                    </tr>
                                    @endforeach
